...
describe.skipIf(!available)('account integration', () => {
  ...
  it('seed backfill on empty org inserts 6 rows and re-run is idempotent (still 6)', async () => {
    ...
    // fetch default company for seed org (created via trigger)
    const { data: seedComp } = await admin.from('company').select('id').eq('organization_id', seedOrgId).limit(1).maybeSingle();
    const seedCompanyId = seedComp?.id;
    expect(seedCompanyId).toBeDefined();
    ...
    ].map((r) => ({ ...r, organization_id: seedOrgId, company_id: seedCompanyId! }));
    ...
    await admin.from('company').delete().eq('organization_id', seedOrgId);
