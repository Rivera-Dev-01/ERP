...
export default async function AppLayout({ children }: { children: React.ReactNode }) {
  ...
  // Reuse React.cache — page-level company fetches dedupe to 0 extra DB round trips per request
  const companies = await getActiveCompanies(organization.id);

  return (
    <div className="flex min-h-screen">
      <Sidebar organizationName={organization.name} companies={companies ?? []} />
