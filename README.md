...
- Single company visible at a time via `?company=`; no cross-company consolidated reports.
...
- Activity history is read-only audit log at `/activity?company=<id>` (no org-wide view, no user filter beyond page).
